        ...
        <div className="space-y-8">
          ...
          <div>
            <h3 className="pl-3 text-xs font-semibold uppercase text-slate-500">
              ...
              <span className="lg:hidden lg:sidebar-expanded:block 2xl:block">Menú Principal</span>
            </h3>
            <ul className="mt-3">
              {/* Listado */}
                <li className={`px-3 py-2 rounded-sm mb-0.5 last:mb-0 ${pathname.includes('job-listing') && 'bg-slate-900'}`}>
                  <NavLink
                    end
                    to="/job/job-listing"
                    className={({ isActive }) =>
                      `block text-slate-200 truncate transition duration-150 ${isActive ? 'hover:text-slate-200' : 'hover:text-white'}`
                    }
                  >
                    <div className="flex items-center">
                      <svg className="w-6 h-6 shrink-0" viewBox="0 0 24 24">
                        <path
                          className={`fill-current ${pathname.includes('job-listing') ? 'text-indigo-500' : 'text-slate-600'}`}
                          d="M18.974 8H22a2 2 0 012 2v6h-2v5a1 1 0 01-1 1h-2a1 1 0 01-1-1v-5h-2v-6a2 2 0 012-2h.974zM20 7a2 2 0 11-.001-3.999A2 2 0 0120 7zM2.974 8H6a2 2 0 012 2v6H6v5a1 1 0 01-1 1H3a1 1 0 01-1-1v-5H0v-6a2 2 0 012-2h.974zM4 7a2 2 0 11-.001-3.999A2 2 0 014 7z"
                        />
                        <path
                          className={`fill-current ${pathname.includes('job-listing') ? 'text-indigo-300' : 'text-slate-400'}`}
                          d="M12 6a3 3 0 110-6 3 3 0 010 6zm2 18h-4a1 1 0 01-1-1v-6H6v-6a3 3 0 013-3h6a3 3 0 013 3v6h-3v6a1 1 0 01-1 1z"
                        />
                      </svg>
                      <span className="ml-3 text-sm font-medium duration-200 lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100">Listado de Ofertas</span>
                    </div>
                  </NavLink>
                </li>
              {/* Inbox */}
              <li className={`px-3 py-2 rounded-sm mb-0.5 last:mb-0 ${pathname.includes('inbox') && 'bg-slate-900'}`}>
                ...
                >
                  <div className="flex items-center">
                    ...
                    <span className="ml-3 text-sm font-medium duration-200 lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100">Mensajes</span>
                  </div>
                </NavLink>
              </li>
            </ul>
                    ...
                    <React.Fragment>
                      ...
                      <div className="lg:hidden lg:sidebar-expanded:block 2xl:block">
                        <ul className={`pl-9 mt-1 ${!open && 'hidden'}`}>
                          ...
                          <li className="mb-1 last:mb-0">
                            <NavLink end to="/reset-password" className="block truncate transition duration-150 text-slate-400 hover:text-slate-200">
                              <span className="text-sm font-medium duration-200 lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100">
                                Reset Password
                              </span>
                            </NavLink>
